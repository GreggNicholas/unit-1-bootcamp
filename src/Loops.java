...
//            12. Modify the method to take in an `int` n and sum the first n Fibonnaci numbers.
  ...
  //#2 Given a string, return a string where every appearance of the lowercase word "is" has been replaced with "is not". The word "is" should not be immediately proceeded or followed by a letter -- so for example the "is" in "this" does not count. (Note: Character.isLetter(char) tests if a char is a letter.)
  ...
  private static String wordEnds(String string, String wordString) {
    int StringLength = string.length();
    int wordStringLength = wordString.length();
    String charString = "";
    for (int i = 0; i < StringLength - wordStringLength + 1; i++) {
      String word = string.substring(i, i + wordStringLength);
      if (i > 0 && word.equals(wordString))
        charString += string.substring(i - 1, i);
      if (i < StringLength - wordStringLength && word.equals(wordString))
        charString += string.substring(i + wordStringLength, i + wordStringLength + 1);
    }
    return charString;
  }
